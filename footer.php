<?php
/**
 * Created by PhpStorm.
 * User: wangr
 * Date: 2018/7/19
 * Time: 14:57
 */
?>

<script type='text/javascript' src='/assets/js/zepto.min.js' charset='utf-8'></script>
<script type='text/javascript' src='/assets/js/zepto.fx.js' charset='utf-8'></script>
<script type='text/javascript' src='/assets/js/sm.min.js' charset='utf-8'></script>
<script type='text/javascript' src='/assets/js/sm-extend.min.js' charset='utf-8'></script>
<!--<script type='text/javascript' src="assets/js/hammer.min.js"></script>-->
<script src="/assets/js/flickity.pkgd.min.js"></script>
<script type="text/javascript" src="/assets/js/tw_cn.js"></script>
<script type="text/javascript" src="/assets/js/RouteMap-master/routemap.js"></script>
<script type="text/javascript" src="/assets/js/scripts.js" charset='utf-8'></script>


<script>
    var baseUrl = document.location.origin + '<?php echo WORKING_DIR == 'mobile'? 'mobile' : ''; ?>';
$.init();
$.initInfiniteScroll('.infinite-scroll-bottom');
</script>



</body>
</html>